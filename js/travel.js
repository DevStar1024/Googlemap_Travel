const rotatesvg = (deg) => 
 `<svg style="transform: rotate(${deg}deg);" version="1.1" id="Layer_2" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="55px" y="75px"
     width="85px" height="85px" viewBox="-45 -50 150 150" enable-background="new -45 -50 150 150"
     xml:space="preserve">
...
function convpath2Position (path, index){
    let loc = path[index].loc1;
    var pos = loc.split(",");
    var path = [];
    var position = {lat: Number(pos[0]), lng: Number(pos[1])};
    return position;

}

 function gettimeStep(strdate1, strdate2){
...
function travel(googleMaps, map, path, id){
    ...
    //set marker
    var vehicle = new googleMaps.Marker({
        position: convpath2Position(path, 0),
        map: map,
        icon: {
            url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(rotatesvg(Number(path[0].angle1)))}`,
            scaledSize: new google.maps.Size(64, 64),
            anchor: new google.maps.Point(32, 32)
        }
    });
    var contentString = '<div style="Width:400px; Height:300px;"><p>Speed<p><p>100m/s</p></div>';
    ...
    var Coordinates = [];
    var polyline  = new googleMaps.Polyline({
        path: Coordinates,
        strokeColor: "#00FF00",
        strokeOpacity: 1.0,
        strokeWeight: 2,
    ...
    var point2;
    var v = 0;
    console.log("123", googleMaps.geometry.spherical.computeDistanceBetween({lat: 37.7749, lng: -122.4194}, {lat: 40.7128, lng:-74.0060 }));
    var interval = setInterval(function() {
        if(convpath2Position(path, 0) == undefined) return;              
        if(path.length == 1) {
            vehicle.setIcon({
                url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(rotatesvg(Number(path[0].angle1)))}`,
                scaledSize: new google.maps.Size(64, 64),
        ...
        point1 =  new googleMaps.LatLng(convpath2Position(path, 0).lat, convpath2Position(path, 0).lng);
        point2 =  new googleMaps.LatLng(convpath2Position(path, 1).lat, convpath2Position(path, 1).lng);
        if(disA == 0){ 
            
            dis = googleMaps.geometry.spherical.computeDistanceBetween(point1, point2);
            var timestep = gettimeStep(path[0].date2, path[1].date2);
            if(timestep == 0) v = 0;
            else v = dis / timestep / 10;
            var heading = googleMaps.geometry.spherical.computeHeading(point1, point2);
            if(v != 0){
                vehicle.setIcon({
                    url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(rotatesvg(heading))}`,
                    scaledSize: new google.maps.Size(64, 64),
                    anchor: new google.maps.Point(32, 32)
                });
            }
        ...
        if(speed_t % 10 == 0){
            totalkilometer += speed;
            updateInfo(infowindow, path, speed, totalkilometer);
            speed = 0;
        }
        position = googleMaps.geometry.spherical.computeOffset(point1, disA, googleMaps.geometry.spherical.computeHeading(point1, point2));
        ...
        if(disA >= dis){
            ...
            //console.log(speed, disA, dis);
            speed = speed - (disA - dis);
            path.shift();
            //i++;
            disA = 0;
        }       
...
function updateInfo(infowindow, path, speed, total){
    //console.log("update");
    infowindow.setContent(`<div style="Width:400px; Height:300px;"><div><h4>${path[0].vno1}</h4></div><div><p>${path[0].tracker1}</p></div><div>(${(convpath2Position(path, 0).lat).toFixed(4)}, ${(convpath2Position(path, 0).lng).toFixed(4)})</div><div><p>Speed<p><p>${(speed * 3.6).toFixed(2)}km/h</p></div><div><p>Total</p><p>${(total/1000).toFixed(2)}km</p></div></div>`);
}